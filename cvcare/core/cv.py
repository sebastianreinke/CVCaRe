...
class FullCV(CV):
    ...
    def _scan_direction_dataset(self, forward_direction: bool, filtered: bool = False):
    ...
    def get_scan_direction_segment(self, forward_direction: bool, filtered: bool = False) -> np.ndarray:
        data_u, data_i = self._scan_direction_dataset(forward_direction, filtered)

        def _mag(value):
            return float(getattr(value, "magnitude", value))

        u_mag = [_mag(v) for v in data_u]
        i_mag = [_mag(v) for v in data_i]
        if not u_mag:
            return np.zeros((0, 2))
        return np.column_stack([np.array(u_mag, dtype=float), np.array(i_mag, dtype=float)])
...
class HalfCV(CV):
    ...
    def get_mirrored_branch(self, filtered: bool = False) -> np.ndarray:
        data = self.get_filtered_dataset() if filtered else self.get_dataset()
        data = np.asarray(data, dtype=float)
        if data.shape[0] < 2:
            raise NotEnoughDataError(
                "This half-cycle has too few points to construct a mirrored branch."
            )
        center = (data[0] + data[-1]) / 2.0
        mirrored = 2.0 * center - data
        return mirrored

    def get_mirrored_full_cv(self, filtered: bool = False) -> np.ndarray:
        real = self.get_filtered_dataset() if filtered else self.get_dataset()
        real = np.asarray(real, dtype=float)
        mirrored = self.get_mirrored_branch(filtered=filtered)
        return np.vstack([real, mirrored])

    def to_virtual_full_cv(self, filtered: bool = False) -> "FullCV":
        virtual_dataset = self.get_mirrored_full_cv(filtered=filtered)
        virtual_cv = FullCV(
            source=f"{self.source} (virtual full CV, mirrored half-cycle)",
            cycle_nr=self.half_cycle_nr,
            index=self.index,
            dataset=virtual_dataset,
            eval_voltage=self.eval_voltage,
            unit_voltage=self.unit_voltage,
            unit_current=self.unit_current,
        )
        if self.scanrate is not None:
            virtual_cv.set_scanrate(self.scanrate, self.unit_scanrate)
        virtual_cv.set_default_filtered(self.default_filtered)
        return virtual_cv
...
def calculate_rc_cv(resistance: float, capacitance: float, period: float,
    ...
    def backward_current(t, A, T, R, C):
        return ((8 * A * C / T) * np.exp((-t + T) / (R * C))
                / (1 + np.exp(T / (2 * R * C))) - 4 * A * C / T)

    half = samples // 2
    t = np.linspace(0.0, float(period), samples)

    forward_vec = np.vectorize(
        partial(forward_current, A=amplitude, R=resistance, C=capacitance,
                T=period)
    )
    reverse_vec = np.vectorize(
        partial(backward_current, A=amplitude, R=resistance, C=capacitance,
                T=period)
    )

    i_forward = forward_vec(t[:half])
    i_reverse = reverse_vec(t[half:])

    interval_fwd = t[half - 1] - t[0]
    interval_rev = t[-1] - t[half]
    rise_fwd = 2 * amplitude / interval_fwd if interval_fwd else 0.0
    rise_rev = 2 * amplitude / interval_rev if interval_rev else 0.0
    v_forward = (t[:half] - t[0]) * rise_fwd
    v_reverse = 2 * amplitude - (t[half:] - t[half]) * rise_rev

    current = np.concatenate([i_forward, i_reverse])
    voltage = np.concatenate([v_forward, v_reverse])
    return np.stack((voltage, current), axis=1)
